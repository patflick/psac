...
public:
    ...
    my_rmq(const my_rmq& o) = default;
    my_rmq(my_rmq&& o) = default;
    my_rmq& operator=(const my_rmq& o) = default;
    my_rmq& operator=(my_rmq&& o) = default;

    my_rmq(Iterator begin, Iterator end,
    ...
    Iterator query(Iterator range_begin, Iterator range_end) const {
        ...
        return _begin + (*this)(begin_idx, end_idx);
    }
        ...
        if (left_sb > right_sb) {
            ...
            if (left_b < right_b) {
                ...
                if (begin_idx < left_b*block_size) {
                    Iterator inblock_min_it = std::min_element(_begin + begin_idx, _begin + left_b*block_size);
                    if (!(*min_pos < *inblock_min_it)) {
                        min_pos = inblock_min_it;
                    }
                }
                if (end_idx > right_b*block_size) {
                    Iterator inblock_min_it = std::min_element(_begin + right_b*block_size, _begin + end_idx);
                    if (*inblock_min_it < *min_pos) {
                        min_pos = inblock_min_it;
                    }
                }
